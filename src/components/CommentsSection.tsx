...
const CommentsSection: React.FC = () => {
  ...
  const getSentimentBadge = (sentiment: string | undefined) => {
    ...
    return (
      classes[sentiment as keyof typeof classes] || "bg-gray-100 text-gray-800"
    );
  };
            ...
            <div key={entry.id} className="bg-white rounded-lg shadow p-6">
              <div className="flex justify-between items-start mb-3">
                <div className="flex gap-3 items-center">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium ${getSentimentBadge(entry.sentiment)}`}
                  >
                    {entry.sentiment}
                  </span>
                </div>
